...
id_to_cookie(Id) ->
    Cookie = cowboy_http:cookie_to_iodata(nopaste_config:cookie(),
                                          Id,
                                          [{path, <<"/">>}]),
    Cookie.
...
get_lifetime() ->
    calendar:datetime_to_gregorian_seconds(calendar:local_time()) + nopaste_config:lifetime().

set_cookie(Req, Id) ->
    ?debugVal(Id),
    Req2 = cowboy_req:set_resp_cookie(nopaste_config:cookie(),
                                      Id,
                                      [{path, <<"/">>}],
                                      Req),
    Req2.
...
signin(#user{id = UserId}, Req) ->
    ...
    nopaste_db:add_session(#session{id = SessionId,
                                    user_id = UserId,
...
get_session(Req) ->
    {Cookie, _Req2} = cowboy_req:cookie(nopaste_config:cookie(), Req),
    ?debugVal(Cookie),
    sanitize_session(nopaste_db:q(dirty_read, [session, Cookie])).

expire(Req) ->
    {Cookie, _Req} = cowboy_req:cookie(nopaste_config:cookie(), Req),
    ?debugVal(Cookie),
    nopaste_db:delete_session(Cookie).
